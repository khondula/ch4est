...
request_udm_url <- function(id_to_use){
  ...
  message(glue("Trying to get {id_to_use}"))
    ...
    if(!is.null(assets[[itemtype]])){
      ...
      message(glue::glue("No {itemtype} item for {id_to_use}"))
    }
...
request_udm_url2 <- function(id_to_use){
  ...
  filename <- glue("{udm_dir}/{id_to_use}_{itemtype}.tiff")
