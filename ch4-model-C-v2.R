...
run_ch4_modelC <- function(my_dep_id, mov_avg_ts_filename, model_runs_id){
# set up for all deps
  ...
  lm1 <- read_rds(glue("{data_dir}/stars/ref/holgerson-lm.rds"))
...
# for just 1 depression
  ...
  # SIZE AREA CONCENTRATION REGRESSION MODEL
  ...
  my_ts_wAreas <- my_ts_wAreas %>% 
    dplyr::mutate(log_area_m2 = log(mvdeps_area))
  
calc_ch4_modelC <- function(my_ts_wAreas){  
  # get distribution given exact area 
  my_ch4_fit <- predict(lm1, newdata = my_ts_wAreas[1,], interval = "confidence", se.fit = TRUE)
  n_samp = 50
  my_sd <- my_ch4_fit$residual.scale
  # my_sd = (my_ch4_fit$se.fit)*sqrt(135)
  # then sample distribution
  # NOT WORKING YET??
  my_ch4_ts <- rlnorm(365, meanlog = my_ch4_fit$fit[1], sdlog = my_sd)
  my_ch4_notfine <- my_ch4_ts %>% purrr::map_lgl(~!is.finite(.x))
  if(any(my_ch4_notfine)){
    my_ch4_mx <- max(my_ch4_ts[is.finite(my_ch4_ts)])
    my_ch4_ts[my_ch4_notfine] <- my_ch4_mx
  }
...
my_runs <- furrr::future_map(my_runs_list, ~calc_ch4_modelC(.x))
message("did future map")
ts_ch4_bind <- my_runs %>% 
  bind_rows(.id = "model_id")

# SAVE CH4 TIME SERIES WITH ALL DEPS
ch4_ts_dir <- glue("{data_dir}/stars/{model_runs_id}/08-ch4-models/outtype_C")
